...
class AuthService {
  ...
  // Login user
  static async login({ idNumber, password, rememberMe }) {
  ...
  // Register new user
  static async register({ name, idNumber, email, password, courseId, rememberMe }) {
    if (!name || !idNumber || !password || !courseId) {
      throw new Error('Name, ID number, password, course ID, and are required');
    }
